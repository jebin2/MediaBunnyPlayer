...
const drawOverlayControls = () => {
	...
	state.mixVideo.forEach((segment, segmentIndex) => {
		segment.video_edit_prop.forEach((prop, rangeIndex) => {
			...
			const start = parseTime(prop.start);
			const end = parseTime(prop.end);
			...
			if (currentTime >= start && currentTime <= end) {
				...
				// Render content to canvas (handles Chroma Key & Animation)
				renderPreviewFrame(box, segment, currentTime - start);
			} else {
			...
			if (segmentIndex === activeSegmentIndex && rangeIndex === activeRangeIndex) {
				...
				box.style.border = '2px solid #00f'; // Make border visible when active
			} else {
...
// --- RENDERING & CHROMA KEY LOGIC ---

const renderPreviewFrame = (box, segment, timeInRange) => {
	const canvas = box.querySelector('canvas');
	if (!canvas) return;

	// Use willReadFrequently to optimize for Chroma Key pixel manipulation
	const ctx = canvas.getContext('2d', { willReadFrequently: true });
	let sourceToDraw = null;
	let srcW = 0, srcH = 0;

	// --- 1. GIF HANDLING (Async Decoder) ---
	if (box._mediaType === 'gif' && box._gifDecoder && box._gifFrameTimestamps.length > 0) {
		const loopTime = timeInRange % box._gifTotalDuration;

		// Find the correct frame index based on time
		let frameIndex = 0;
		for (let i = 0; i < box._gifFrameTimestamps.length; i++) {
			if (loopTime >= box._gifFrameTimestamps[i]) {
				frameIndex = i;
			} else {
				break;
			}
		...
		// If we haven't drawn this frame yet, decode and draw it
		if (box._lastRenderedFrameIndex !== frameIndex && !box._isDecodingGif) {
			box._isDecodingGif = true;
			box._gifDecoder.decode({ frameIndex: frameIndex })
				.then(result => {
					const frame = result.image;
					srcW = frame.displayWidth;
					srcH = frame.displayHeight;
					...
					// Apply Chroma Key
					if (segment.chromaKey && segment.chromaKey.color) {
						applyChromaKeyToCanvas(ctx, srcW, srcH, segment.chromaKey);
					}

					box._lastRenderedFrameIndex = frameIndex;
					box._isDecodingGif = false;
				})
				.catch(e => {
		...
		// We return early for GIFs because the drawing happens inside the promise
		return;
	}

	// --- 2. VIDEO & STATIC IMAGE HANDLING ---
	const sourceEl = box._sourceElement;
	if (!sourceEl) return;

	// Sync Video Time
	if (box._mediaType === 'video') {
		if (sourceEl.readyState >= 2) {
			const duration = sourceEl.duration || 1;
			const overlayTime = timeInRange % duration;
			// Only seek if deviation is significant (prevents jitter)
			if (Math.abs(sourceEl.currentTime - overlayTime) > 0.1) {
				sourceEl.currentTime = overlayTime;
			}
		}
	}

	srcW = (sourceEl.videoWidth || sourceEl.naturalWidth || sourceEl.width);
	srcH = (sourceEl.videoHeight || sourceEl.naturalHeight || sourceEl.height);
	...
	if (srcW > 0 && srcH > 0) {
		...
		ctx.clearRect(0, 0, srcW, srcH);
		ctx.drawImage(sourceEl, 0, 0, srcW, srcH);
...
const applyChromaKeyToCanvas = (ctx, width, height, settings) => {
	...
	const { r: keyR, g: keyG, b: keyB } = hexToRgb(settings.color);
...
function createOverlayBox(segmentIndex, rangeIndex) {
	...
	const segment = state.mixVideo[segmentIndex];
	...
	// Remove background color and border, the content will define the shape now
	// But keep a thin border to see it if image is black
	box.style.border = '1px dashed rgba(255, 255, 255, 0.5)';

	// --- 1. CREATE DISPLAY CANVAS ---
	// This is what the user sees. We render everything here.
	...
	box._blobUrl = blobUrl;

	// A. Video Handling
	if (segment.media_type === 'mix_video') {
		box._mediaType = 'video';
		...
		box._sourceElement = video;
	}
	// B. GIF Handling (Animated)
	else if (segment.media_type === 'mix_gif') {
		box._mediaType = 'gif';
		box._gifDecoder = null;
		box._gifFrameTimestamps = [];
		box._gifTotalDuration = 0;

		// Immediate Fallback (Static Image)
		const img = document.createElement('img');
		img.src = blobUrl;
		img.style.display = 'none';
		box.appendChild(img);
		box._sourceElement = img; // Use as fallback

		// Background Decoder Setup
			...
			file.arrayBuffer().then(async (buffer) => {
				try {
					...
					for (let i = 0; i < decoder.tracks.selectedTrack.frameCount; i++) {
						...
						const frameDur = (res.image.duration || 100000) / 1_000_000;
						currentTime += frameDur;
						res.image.close();
					}

					box._gifFrameTimestamps = timestamps;
					box._gifTotalDuration = currentTime;
					box._gifDecoder = decoder;

				} catch (e) {
					console.error("GIF Decoder Init Failed", e);
				}
			});
		}
	}
	// C. Static Image Handling
	else {
		box._mediaType = 'image';
		const img = document.createElement('img');
		img.src = blobUrl;
		img.style.display = 'none';
		box.appendChild(img);
		box._sourceElement = img;
	}
	...
	['n', 's', 'e', 'w', 'ne', 'nw', 'se', 'sw'].forEach(handle => {
		...
		handleEl.style.zIndex = '1001'; // Higher than the image
